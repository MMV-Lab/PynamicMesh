...
def edit_graph(mesh_folder_path, reeb_folder_path):
    ...
    def update_frame(frame_idx):
        ...
        if G.number_of_nodes() > 0:
            ...
            if sel_node is not None and G.has_node(sel_node):
                sel_pos = G.nodes[sel_node]['pos']
                sel_pv = pv.PolyData(np.array([sel_pos]))
                plotter.add_mesh(
                    sel_pv.glyph(geom=pv.Sphere(radius=point_size * 1.5), scale=False, orient=False),
        ...
        instruction_text = (
            ...
            "  - [I] Inner Mode: Click node to push it inside mesh (raycast limit).\n"
            "  - [O] Outer Mode: Click node to pull it outside (limit=surface).\n"
            ...
            if mode == 'edge_delete':
                ...
                if clicked_node_id is not None:
                    ...
                    if mode == 'link':
                        ...
                        node_data = G.nodes[clicked_node_id]
                        
                        if 'orig_pos' not in node_data:
                            node_data['orig_pos'] = node_data['pos'].copy()
                            v_idx = mesh_pv.find_closest_point(node_data['pos'])
    ...
    plotter.add_key_event('d', lambda: set_mode('edge_delete')) # New Edge Delete Toggle
...
def graph_time_analysis(reeb_folder_path,single_file=True):
    ...
    for i in tqdm(range(1, len(reeb_files)), desc="Computing Graph Metrics"):
        with open(reeb_files[i], 'rb') as f:
...
def launch_reeb_viewer(mesh_files, reeb_files, scalar_files):
    ...
    def update_frame(frame_idx):
        ...
        if reeb_pv.n_points > 0:
            spheres = reeb_pv.glyph(geom=pv.Sphere(radius=0.005), scale=False, orient=False)
            pl.add_mesh(spheres, color='red', name='reeb_nodes', render=False)
            
            if reeb_pv.n_lines > 0:
                tubes = reeb_pv.tube(radius=0.002) 
